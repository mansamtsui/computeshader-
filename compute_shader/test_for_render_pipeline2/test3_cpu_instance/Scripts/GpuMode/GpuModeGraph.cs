...
public class GpuModeGraph : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        // 必须 塞满每个线程组，免得 Compute Shader 中有线程读不到数据，造成读取数据越界
    ...
    private void UpdatePointsPosition()
    {
        ...
        //可以通过这个方法获得CS中[numthreads(8,8,1)]
        uint x;
        // 获取 Compute Shader 中定义的 numthreads
        m_ComputeShader.GetKernelThreadGroupSizes(m_KernelIndex, out x, out _, out _);
       // groups = Mathf.CeilToInt(m_Resolution / (float)x);
